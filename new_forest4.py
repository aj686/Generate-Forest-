...
# Define a function to calculate the damage to the crown
def calculate_damage_crown(row):
    if row['Status'] == 'Cut' and row['Diameter (cm)'] > 45:
        cutting_angle = row['Cutting Angle']

        sin_value = math.sin(math.radians(cutting_angle))
        stem_x = (row['Height (m)'] + 5) * sin_value

        cos_value = math.cos(math.radians(cutting_angle))
        stem_y = (row['Height (m)'] + 5) * cos_value

        damage_stem = max(0, math.sqrt((stem_x)**2 + (stem_y)**2))  # Ensure damage_stem is non-negative
        row['Damage Stem'] = f'{damage_stem:.2f}'
    else:
        row['Damage Stem'] = "N/A"

    new_coordinate_x = row['Coordinate_X']
    new_coordinate_y = row['Coordinate_Y']
    cutting_angle = row['Cutting Angle']
    
    Y1 = new_coordinate_x / math.tan(math.radians(cutting_angle) + 1)
    Y2 = new_coordinate_x / math.tan(math.radians(cutting_angle) - 1)

    # Ensure Y1 and Y2 are valid values
    Y1 = max(0, Y1)
    Y2 = max(0, Y2)

    if Y1 < new_coordinate_y < Y2:
        row['Status'] = 'Cut'
        row['Damage Crown'] = f'{(Y1 + Y2) / 2:.2f}'
    else:
        row['Damage Crown'] = 'N/A'

    return row['Damage Crown']
...
def calculate_production_30(diameter):
    ...
    return round(0.0971 + 9.503 * D**2, 2)
    ...
    for block_y in tqdm(range(1, blockSizeY + 1), desc="Blocks Y", leave=False):
        ...
        for species_group, trees_in_group in enumerate(trees_per_hectare, start=1):
            ...
            for _ in range(trees_in_group):
                ...
                diameter = round(random.uniform(diameter_ranges[0][0], diameter_ranges[-1][1]), 2)
                height = round(random.uniform(height_ranges[0][0], height_ranges[-1][1]) / 100, 2)

                # Calculate volume of the tree
                volume = round(3.142 * (diameter / 200) ** 2 * (height * 0.50), 2)
                ...
                tree_data.append([
                    ...
                    f'{volume:.2f}', 
                    status, 
                    f'{calculate_production(diameter):.2f}',  # Production
                    f'{volume:.2f}' if status == 'Cut' else "", 
                    cutting_angle,
                    calculate_damage_crown({
                        'Status': status,
                        'Diameter (cm)': diameter,  # Include Diameter (cm) key
                        'Height (m)': height,       # Include Height (m) key
                    ...
                    f'{calculate_growth(diameter)[-1]:.2f}',  # Add the diameter after 30 years of growth
                    f'{calculate_production_30(diameter):.2f}',  # Add the Production 30
                    f'{diameter_30:.2f}',  # Diameter after 30 years
                    f'{volume_30:.2f}',  # Volume after 30 years
                ])
